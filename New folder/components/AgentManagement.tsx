...
const AgentManagement: React.FC<AgentManagementProps> = ({ agents, onAgentsChange }) => {
  ...
  const handleDeleteAgent = (id: string) => {
    ...
    if (window.confirm(`هل أنت متأكد أنك تريد حذف الوكيل '${agentToDelete.name}'؟`)) {
                ...
                agents.map(agent => (
                  <div key={agent.id} className="bg-gray-700/50 p-3 rounded-lg flex justify-between items-start">
                    ...
                    <div className="flex-shrink-0 flex items-center">
                        <button onClick={() => handleEditAgent(agent.id)} className="p-2 text-gray-400 hover:text-cyan-400" title="تعديل"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fillRule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clipRule="evenodd" /></svg></button>
                        <button onClick={() => handleDeleteAgent(agent.id)} className="p-2 text-gray-400 hover:text-red-500" title="حذف"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                    </div>
                  </div>
                ))
              ) : (
